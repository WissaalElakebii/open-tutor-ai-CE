...
from data.models import User, Support
...
async def get_dashboard(
    ...
    return {
        ...
        "activite_recente": [
            {"type": "evaluation", "titre": "Mathématiques — Équations", "score": 89, "date": "2026-06-22T14:32:00"},
            {"type": "session_ia", "titre": "Session IA — Algèbre", "duree_min": 38, "date": "2026-06-22T11:05:00"},
            {"type": "module", "titre": "Module Français — Conjugaison", "date": "2026-06-21T16:45:00"},
            {"type": "soutien", "titre": "Soutien Physique activé", "date": "2026-06-21T09:10:00"},
        ],
        ...
        "notifications": [
            {"type": "resultat", "titre": "Nouveau résultat — Maths", "desc": "89/100 — meilleur score", "date": "Il y a 1h",   "lu": False},
            {"type": "soutien",  "titre": "Soutien terminé — Physique",  "desc": "Complété le 10 juin",   "date": "Hier",        "lu": False},
            {"type": "ia",       "titre": "Recommandation IA",            "desc": "Renforcer les fractions","date": "Il y a 2j",  "lu": False},
            {"type": "alerte",   "titre": "Alerte — SVT en baisse",       "desc": "3 modules non validés", "date": "Il y a 4j",  "lu": True},
        ],
    }
...
async def get_sessions(
    ...
    sessions = [
        {"id":"s1","date":"2026-06-15","matiere":"Mathématiques","titre":"Équations du 2nd degré","duree_min":38,"themes":["Discriminant","Factorisation","Résolution"],"questions":["Comment calculer le discriminant avec des fractions ?","Pourquoi quand Δ<0 il n'y a pas de solution ?","Peut-on factoriser si Δ=0 ?"],"resume":"Yassine maîtrise les 3 cas. Point de vigilance : erreurs de signe lors du calcul de b². Recommandation : exercices sur coefficients fractionnaires.","score_qualite":9.1,"engagement":9.2,"comprehension":8.8,"autonomie":8.0,"statut":"complete"},
        {"id":"s2","date":"2026-06-14","matiere":"Français","titre":"Analyse de texte — Le Naturalisme","duree_min":45,"themes":["Littérature","Zola","Argumentation"],"questions":["Différence entre réalisme et naturalisme ?","Comment structurer une introduction ?","C'est quoi la 'tranche de vie' ?"],"resume":"Excellente session. Yassine distingue bien réalisme et naturalisme, a produit un paragraphe d'analyse structuré.","score_qualite":8.7,"engagement":9.5,"comprehension":8.5,"autonomie":8.2,"statut":"complete"},
        {"id":"s3","date":"2026-06-12","matiere":"Physique-Chimie","titre":"Forces et vecteurs","duree_min":22,"themes":["Vecteurs","Forces"],"questions":["Je ne comprends pas comment dessiner un vecteur force...","C'est quoi la différence entre poids et masse ?"],"resume":"Session interrompue. Difficultés sur les vecteurs forces. La notion poids/masse reste floue. Recommandation : créer un soutien IA Physique.","score_qualite":6.4,"engagement":5.5,"comprehension":6.0,"autonomie":4.2,"statut":"partielle"},
        {"id":"s4","date":"2026-06-11","matiere":"Anglais","titre":"Past Simple & Present Perfect","duree_min":31,"themes":["Grammaire","Temps verbaux"],"questions":["Quand utilise-t-on have been vs was/were ?","Est-ce que since va toujours avec le present perfect ?"],"resume":"Bonne session. Yassine comprend la distinction Past Simple / Present Perfect. 8/10 exercices réussis.","score_qualite":8.3,"engagement":8.5,"comprehension":8.2,"autonomie":7.8,"statut":"complete"},
    ]
    ...
    stats = {
        ...
        "temps_total": "11h20",
        "score_qualite_moyen": round(sum(s["score_qualite"] for s in sessions) / len(sessions), 1) if sessions else 0,
        "total_questions": 147,
    }
...
async def get_notifications(
    ...
    notifications = [
        {"id":"n1","type":"resultat","titre":"Nouveau résultat — Mathématiques","desc":"Yassine a obtenu 89/100 à l'évaluation Équations du 2nd degré. ▲ +11 pts.","date":"Il y a 1h30","action_url":"/parent/evaluations","lu":False},
        {"id":"n2","type":"ia","titre":"Recommandation IA personnalisée","desc":"Suite à la session d'aujourd'hui, le tuteur IA recommande de renforcer les fractions avant les équations complexes.","date":"Il y a 2h","action_url":"/parent/support/create","lu":False},
        {"id":"n3","type":"soutien","titre":"Soutien terminé","desc":"Le soutien Fractions et opérations a été complété. Progression : +15 pts.","date":"Il y a 3h","action_url":"/parent/sessions","lu":False},
        {"id":"n4","type":"alerte","titre":"Alerte progression — SVT","desc":"La progression en SVT est en baisse. Score moyen : 58/100. 3 modules non validés.","date":"Hier","action_url":"/parent/support/create","lu":True},
        {"id":"n5","type":"resultat","titre":"Nouveau résultat — Anglais","desc":"Yassine a obtenu 74/100 à Reading Comprehension. ▲ +4 pts.","date":"Hier","action_url":"/parent/evaluations","lu":True},
        {"id":"n6","type":"rapport","titre":"Rapport hebdomadaire — Semaine 24","desc":"6 sessions IA (3h40), 2 évaluations, 1 module terminé. Score en hausse de +4 pts.","date":"Il y a 5j","action_url":"/parent/dashboard","lu":True},
    ]
...
async def get_support_progress(
    ...
    if not support.chat_id:
        return {"support_id": support_id, "progress": 0, "messages_count": 0, "status": "pending"}

    chat = db.query(Chat).filter(Chat.id == support.chat_id).first()
    if not chat or not chat.chat:
        return {"support_id": support_id, "progress": 0, "messages_count": 0, "status": "active"}

    # Compter les messages assistant (réponses IA)
    chat_data = chat.chat or {}
    messages = chat_data.get("messages", {})
    if isinstance(messages, dict):
        ai_messages = [m for m in messages.values() if isinstance(m, dict) and m.get("role") == "assistant"]
    elif isinstance(messages, list):
        ai_messages = [m for m in messages if isinstance(m, dict) and m.get("role") == "assistant"]
    else:
        ai_messages = []
...
async def get_real_sessions(
    ...
    for support in supports:
        ...
        avg_len = sum(len(str(m.get("content", ""))) for m in ai_messages) / max(ai_count, 1)
        ...
        for m in user_messages[:3]:
            ...
            if content and len(content) > 3:
                questions.append(str(content)[:120])
        ...
        if ai_messages:
            ...
            resume = str(last_content)[:300] if last_content else ""
    ...
    stats = {
        ...
        "score_qualite_moyen": round(sum(s["score_qualite"] for s in sessions) / max(len(sessions), 1), 1),
